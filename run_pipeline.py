...
from src.spatial_stats import compute_moran, compute_getis_gi_star, compute_kde_intensity
...
def run_pipeline(year: int = 2025, hex_diameter: float = 100.0):
    # STEP 1: Boundary
    ...
    print(f"=== STEP 3: Aggregating crime + environmental features ({year}) ===\n")
    ...
    # STEP 6: GWR or Local Linear

    print("=== STEP 6: Fitting GWR ===")
    try:
        if len(features_gdf) > 6000:
            print("Skipping MGWR (grid too large). Using Local Linear fallback.")
    ...
    # STEP 7: Spatial stats (Moran, Gi*, KDE)

    print("=== STEP 7: Spatial statistics (Moran, Gi*) and KDE ===")
    ...
    features_gdf = compute_kde_intensity(features_gdf, bandwidth=200.0)
    print()
    ...
    history, forecast, forecast_path = forecast_monthly_crime(monthly, horizon=6)
...
if __name__ == "__main__":
    run_pipeline(year=2025, hex_diameter=100.0)